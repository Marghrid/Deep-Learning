...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        relu_affine, cache_relu_affine = affine_relu_forward(X, self.params["W1"], self.params["b1"])
        scores, cache_f = affine_forward(relu_affine, self.params["W2"], self.params["b2"])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        d_relu_affine, grads["W2"], grads["b2"] = affine_backward(d_scores, cache_f)
        dX, grads["W1"], grads["b1"] = affine_relu_backward(d_relu_affine, cache_relu_affine)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for idx in range(self.num_layers):
            ...
            elif idx == self.num_layers - 1: #last layer
            ...
            self.params["W%d" % (idx + 1)] = np.random.normal(0.0, weight_scale, (in_dim, out_dim))
            self.params["b%d" % (idx + 1)] = np.zeros(out_dim, dtype=float)
    ...
    def loss(self, X, y=None):
        ...
        for idx in range(self.num_layers):
            ...
            if idx == self.num_layers - 1: #  last layer - no relu
                out, cache = affine_forward(out, w, b)

            elif self.use_batchnorm: # insert a batch normalization layer before each ReLU nonlinearity
